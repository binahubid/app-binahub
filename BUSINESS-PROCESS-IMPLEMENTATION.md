...
- **Fase 4 — Automation Control & Production Hardening:** implementasi kode utama tersedia pada working copy v0.10.0; migration `0029`, deployment, n8n import, dan UAT masih menunggu.

Dengan demikian, pekerjaan saat ini berada pada Fase 4 pra-deployment. Aktivasi otomatis penuh tetap terkunci dan seluruh worker terjadwal tetap memakai dry-run sampai UAT disetujui.
...
2. Pastikan migration `0023_business_process_p0.sql`–`0028_client_delivery_and_retention.sql` telah diterapkan, lalu terapkan `0029_automation_control_and_human_tasks.sql` melalui prosedur di `binahub-api/supabase/DEPLOYMENT.md`.
...
5. Deploy berurutan: `binahub-api` v0.10.0 → `app-binahub` v0.10.0. `website-prod` tidak berubah pada iterasi Fase 4 ini.
